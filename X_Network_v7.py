...
class ConvTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, k=3):
        ...
        # Simplified dimension reducer without precision casting
    ...
    def forward(self, src, mask, src_mask=None):
        ...
        # Use nearest neighbor interpolation for discrete structural data
        if mask.shape[-2:] != (seq_len, seq_len):
            mask = F.interpolate(mask, size=(seq_len, seq_len), mode='nearest', align_corners=None)
        ...
        deconv_mask = F.interpolate(
            ...
            mode='nearest'  # Use nearest for discrete structures
        )
...
class RNADegformer(nn.Module):
    ...
    def __init__(self, ntoken, nclass, ninp, nhead, nhid, nlayers,
                 stride=1, dropout=0.5, pretrain=False, return_aw=False, rinalmo_weights_path=None,
                 precomputed_features_path='data/precomputed_features.pt'):
        self._precomputed_features_path = precomputed_features_path
        super(RNADegformer, self).__init__()
        self.model_type = 'Transformer'
        self.src_mask = None
        # self.kmers = kmers - no longer used

        # ARCHITECTURAL SIMPLIFICATION: Remove unused RiNALMo instance
        # The model now uses precomputed embeddings exclusively
        # RiNALMo is handled centrally in serialize_embeddings_v7.py

        # CRITICAL FIX: Dynamic embedding dimension loading for mega model compatibility
        precomputed_features_path = getattr(self, '_precomputed_features_path', 'data/precomputed_features.pt')
        if os.path.exists(precomputed_features_path):
            feat_data = torch.load(precomputed_features_path, weights_only=False)
            rinalmo_embedding_dim = feat_data.get('embedding_dim', feat_data['embeddings'].shape[-1])
            print(f"✅ Dynamic embedding dimension loaded: {rinalmo_embedding_dim}")
        else:
            rinalmo_embedding_dim = 640  # Emergency fallback
            print(f"⚠️ Using fallback embedding dimension: {rinalmo_embedding_dim}")

        # Store embedding dimension as instance attribute for external access
        self.rinalmo_embedding_dim = rinalmo_embedding_dim

        self.projection = nn.Sequential(
            nn.Linear(rinalmo_embedding_dim, ninp),
            nn.LayerNorm(ninp)
        )

        self.transformer_encoder = []
        # pick a fixed local context window—for example, a 5×1 filter—
        # to inject positional inductive bias before each attention layer
        kernel_size = 5
        for _ in range(nlayers):
            self.transformer_encoder.append(
                ConvTransformerEncoderLayer(ninp, nhead, nhid, dropout, k=kernel_size)
                )
        ...
        # ΔG scalar projection & joint decoder
        self.dg_proj = nn.Sequential(
            nn.Linear(1, ninp // 4),
            nn.ReLU()
        )
        self.joint_decoder = nn.Linear(ninp + (ninp // 4), nclass)

        # Structural-feature projections
        # graph_dist: (B, L, L) → embed via 1×1 Conv2d
        self.graph_proj = nn.Sequential(
            nn.Conv2d(1, ninp // 4, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(ninp // 4, ninp // 4, kernel_size=1)
        )
        # nearest_p and nearest_up: (B,2,L) → embed via 1×1 Conv1d
        self.seq_proj = nn.Sequential(
            nn.Conv1d(2, ninp // 4, kernel_size=1),
            nn.ReLU(),
            nn.Conv1d(ninp // 4, ninp // 4, kernel_size=1)
        )
        # combine and project to ninp
        self.struct_proj = nn.Linear((ninp // 4) * 2, ninp)
    ...
    def forward(self,
        ...
        # Fuse in structural features
        # 1) Project graph-distance matrix
        gd = graph_dist.unsqueeze(1)  # (B,1,L,L)
        gd_emb = self.graph_proj(gd).mean(dim=-1)  # (B, Cs, L)

        # 2) Project nearest_p / nearest_up vectors
        np_feat = torch.stack([nearest_p, nearest_up], dim=1)  # (B,2,L)
        np_emb = self.seq_proj(np_feat)  # (B, Cs, L)

        # 3) Concatenate and project to ninp
        struct_cat = torch.cat([gd_emb, np_emb], dim=1).permute(0, 2, 1)  # (B, L, 2*Cs)
        struct_emb = self.struct_proj(struct_cat)  # (B, L, ninp)

        # 4) Add into the base embeddings
        embeddings = embeddings + struct_emb

        original_mask = bpp  # explicitly preserve original structural tensor (4 channels)
        mask = original_mask

        # FIXED: Allow proper mask evolution through transformer layers
        attention_maps = []
        for i, layer in enumerate(self.transformer_encoder):
            embeddings, attn_weights, updated_mask = layer(embeddings, mask, src_mask[:, i])
            attention_maps.append(attn_weights)  # shape: [B, n_heads, L, L]

            # FIXED: Use evolved mask for subsequent layers instead of resetting
            if updated_mask is not None:
                mask = updated_mask
            # Remove the reset to original_mask - allow mask evolution
        ...
        if self.pretrain:
            ...
            if deltaG is None:
                ...
                # project ΔG scalar → [B, d']
                dg_h = self.dg_proj(deltaG.unsqueeze(-1).to(embeddings.dtype))
                # expand to [B, L, d']
                seq_len = embeddings.size(1)
                dg_exp = dg_h.unsqueeze(1).expand(-1, seq_len, -1)
                # concat → [B, L, ninp + d']
                emb_comb = torch.cat([embeddings, dg_exp], dim=-1)
                logits = self.joint_decoder(emb_comb)
            return logits.float()