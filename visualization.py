...
from Dataset_v6 import RNADataset, variable_length_collate_fn
...
def visualize_best_model(
    ...
    dataset = RNADataset(json.sequence.to_list(), np.zeros(len(json)), ids,
                         np.arange(len(json)), bpp_path='./data/kaggle/new_sequences_bpps/', pad=True, k=5)
    ...
    model = RNADegformer(
        ...
        pretrain=True, rinalmo_weights_path=rinalmo_path
    ).to(device)
    ...
    with torch.no_grad():
        outputs, attention_maps = model(masked_embeddings, bpp, src_mask)
