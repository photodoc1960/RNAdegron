...
def validate(model, device, val_dataloader, batch_size):
    ...
    with torch.no_grad():
        for data in val_dataloader:
            ...
            # CRITICAL FIX: Handle unified BPP tensor structure consistently
            # Expect BPPs in unified format: (batch_size, 4, seq_len, seq_len) or 
            # (batch_size, num_variants, 4, seq_len, seq_len)
            
            if bpps.dim() == 5:  # (batch_size, num_variants, 4, seq_len, seq_len)
                # Select first variant for validation
                bpps = bpps[:, 0, :, :, :]  # -> (batch_size, 4, seq_len, seq_len)
            elif bpps.dim() == 3:  # (batch_size, seq_len, seq_len) - legacy format
                print("Warning: Received legacy BPP format, converting to unified structure")
                batch_size, seq_len = bpps.shape[0], bpps.shape[1]
                # Create 4-channel structure with distance masks
                dm = get_distance_mask(seq_len)
                dm_tensor = torch.tensor(dm).unsqueeze(0).repeat(batch_size, 1, 1, 1).to(device)
                bpps = torch.cat([bpps.unsqueeze(1), dm_tensor], dim=1)
            elif bpps.dim() == 4 and bpps.shape[1] != 4:
                raise ValueError(f"Unexpected BPP tensor shape: {bpps.shape}. Expected 4 channels in dimension 1.")
            
            # Validate final BPP structure
            if bpps.dim() != 4 or bpps.shape[1] != 4:
                raise ValueError(f"BPP tensor must have shape (batch_size, 4, seq_len, seq_len), got {bpps.shape}")

            # Handle source mask dimensions consistently
            ...
            # CRITICAL: Check for NaN explicitly here
            if torch.isnan(loss):
    ...
    val_loss /= max(step, 1)  # Avoid division by zero explicitly
