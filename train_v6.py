...
# ─── Embed precomputed RiNALMo features into the base RNADataset ──────────────────
...
def load_pretrained_into_dataparallel(model, checkpoint_path):
    ...
    if list(state_dict.keys())[0].startswith('module.'):
        ...
        # allow missing params for the new proj layers
...
def train_fold():
    ...
    # load precomputed embeddings (with full unpickling)
    precomp        = torch.load(os.path.join(opts.path, 'precomputed_features.pt'), weights_only=False)
    ids_all        = precomp['ids']
    emb_all        = precomp['embeddings']
    embedding_dict = dict(zip(ids_all, emb_all))

    # base dataset remains the same, then wrap to inject 'embedding'
    base_dataset = RNADataset(
        train_seqs, train_labels, train_ids, train_ew,
        opts.path, pad=True, training=True
    )
    dataset = EmbeddedRNADataset(base_dataset, embedding_dict)

    base_val_dataset = RNADataset(
        val_seqs, val_labels, val_ids, val_ew,
        opts.path, pad=True, training=True
    )
    val_dataset = EmbeddedRNADataset(base_val_dataset, embedding_dict)
    ...
    for epoch in range(opts.epochs):
        ...
        for data in dataloader:
            ...
            with autocast(dtype=torch.float16):
                ...
                loss = criterion(output[:, :68], labels, ew)
