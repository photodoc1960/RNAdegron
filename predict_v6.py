...
from Dataset_v6 import Dataset, variable_length_collate_fn
...
class RNADataset(Dataset):
    def __init__(self, seqs, labels, ids, ew, bpp_path, transform=None, training=True, pad=False, k=5, num_layers=6):
        ...
        self.pad = pad
        # Compute deltaG for all sequences
        print("Computing ΔG values for prediction dataset...")
        self.deltaG_values = []
        for i, seq in enumerate(self.seqs):
            try:
                seq_clean = seq.replace('T', 'U')
                _, mfe = RNA.fold(seq_clean)
                self.deltaG_values.append(float(mfe))
            except Exception as e:
                print(f"Warning: ΔG computation failed for sequence {i}: {e}")
                self.deltaG_values.append(0.0)

            if (i + 1) % 100 == 0:
                print(f"  Processed {i + 1}/{len(self.seqs)} sequences")
    ...
    def __getitem__(self, idx):
        sample = {
            ...
            'deltaG': torch.tensor(self.deltaG_values[idx], dtype=torch.float32)
        }
        return sample
...
def process_with_sliding_window(fold_models, sequence, bpps, sid, window_size=130, stride=65):
        ...
        for model in fold_models:
            with torch.no_grad():
                pred = model(
                    ...
                    src_mask_batch,
                    None,  # deltaG
                    graph_dist_dict[sid][start_idx:end_idx, start_idx:end_idx],
                    nearest_p_dict[sid][start_idx:end_idx],
                    nearest_up_dict[sid][start_idx:end_idx]
                )
            window_preds.append(pred)
...
# create output dir if needed (no error if it already exists)
...
# Load precomputed RiNALMo features (embeddings + structural) with full unpickling
# locate precomputed features (lookup in opts.path first, then fallback to data/)
feat_path = os.path.join(opts.path, 'precomputed_features.pt')
if not os.path.exists(feat_path):
    alt = os.path.join('data', 'precomputed_features.pt')
    if os.path.exists(alt):
        feat_path = alt
    else:
        raise FileNotFoundError(
            f"Could not find precomputed_features.pt (tried {feat_path} and {alt})"
        )
feat_data = torch.load(feat_path, weights_only=False)

# Build dicts for embeddings + structural features
...
embedding_dict   = {str(s): e for s,e in zip(ids_all,      embs_all)}
graph_dist_dict  = {str(s): gd for s,gd in zip(ids_all,    graph_all)}
nearest_p_dict   = {str(s): p  for s,p  in zip(ids_all,    npd_all)}
nearest_up_dict  = {str(s): u  for s,u  in zip(ids_all,    nud_all)}
...
if ls_indices.sum() > 0:
    ...
    if ls_indices.sum() > 0:
        ...
        long_dataset = BPPSafeDataset(EmbeddedRNADataset(
            original_long_dataset,
            embedding_dict,
            graph_dist_dict,
            nearest_p_dict,
            nearest_up_dict
...
if ss_indices.sum() > 0:
    ...
    short_dataset = BPPSafeDataset(EmbeddedRNADataset(
        original_short_dataset,
        embedding_dict,
        graph_dist_dict,
        nearest_p_dict,
        nearest_up_dict
...
with torch.no_grad():
    for batch in tqdm(long_dataloader, desc="Processing long sequences"):
        ...
        if seq_len > 130:
            sid = sample_id[0] if isinstance(sample_id, (list, np.ndarray)) else sample_id
            windows = process_with_sliding_window(
                fold_models, sequences, bpp_single, sid, window_size=130, stride=65
            )
            merged = torch.zeros(
            ...
            # Process with all fold models using existing tensor structure
            outputs = []
            for model in fold_models:
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    pred = model(
                        ...
                        None,  # deltaG
                        batch['graph_dist'].to(device),
                        batch['nearest_p'].to(device),
                        batch['nearest_up'].to(device)
                    )
                outputs.append(pred.cpu())

            # Stack to expected shape: (num_folds, batch_size, seq_len, classes)
            preds.append(torch.stack(outputs, dim=0).numpy())
...
# Combine predictions (preserve original mathematical pattern)
ids_combined = ids + ids_short
preds_combined = []

# Original tensor extraction pattern
for batch_preds in preds + preds_short:
    batch_size = batch_preds.shape[1]
    for batch_idx in range(batch_size):
        # Extract: (num_folds, seq_len, classes)
        preds_combined.append(batch_preds[:, batch_idx, :, :])

# Organize predictions by test sequence
...
# Original prediction averaging (restore mathematical consistency)
to_csv = []
for i in tqdm(range(len(preds_to_csv))):
    if preds_to_csv[i]:
        # Extract tensor: (num_folds, seq_len, classes)
        pred_tensor = preds_to_csv[i][0]

        # Average across folds: (seq_len, classes)
        to_write = np.mean(pred_tensor, axis=0)

        # Append each position's prediction
        for vector in to_write:
            to_csv.append(vector)
    else:
        raise ValueError(f"Missing predictions for sequence {i}")
