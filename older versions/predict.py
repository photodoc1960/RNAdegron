...
import argparse
from train_pl import RNADatasetPrecomputed
...
class BPPSafeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 🔍 Fix case where bpp has wrong dimensions: [1, 1, L, L] or [1, L, L] or [4, L]
        if isinstance(bpp, np.ndarray):
            ...
            if bpp.ndim == 3 and bpp.shape[0] == 1:
                # [1, L, L] → repeat to [4, L, L]
                bpp = np.repeat(bpp, 4, axis=0)

            if bpp.ndim == 3 and bpp.shape[0] == 4 and bpp.shape[1] == bpp.shape[2]:
                sample['bpp'] = bpp  # Valid, assign it back

            else:
                raise ValueError(f"[BPPSafeDataset] Invalid BPP shape after fix: {bpp.shape}")

        else:
            raise TypeError(f"[BPPSafeDataset] BPP is not a NumPy array: type {type(bpp)}")
    ...
    if seq_len <= window_size:
        ...
        for model in fold_models:
            with torch.no_grad():
                ...
                num_layers = len(model.module.transformer_encoder)
                src_mask = torch.ones((batch_size, num_layers, seq_len), device=sequences.device).bool()

                pred = model(sequence, bpps[0:4], src_mask)
    ...
    for start_idx in range(0, seq_len - window_size + 1, stride):
        ...
        for model in fold_models:
            with torch.no_grad():
                ...
                pred = model(seq_window, bpp_window, src_mask)
...
def get_args():
    ...
    parser.add_argument('--rinalmo_weights_path', type=str, default='', help='path for pretrained RNADegformer weights')
    parser.add_argument('--stride', type=int, default=1,
                        help='Stride value explicitly used for RNA Degformer model (default: 1)')
...
# Modify predict.py at line 76-95 (the model loading section)
fold_models=[]
folds=np.arange(opts.nfolds)
for fold in folds:
    # Instead of loading 5 models and averaging them, just load the best model
...
if ls_indices.sum() > 0:
    ...
    print("Forcing regeneration of BPP matrices for all long sequences...")

    bpps_save_dir = os.path.join(opts.path, 'new_sequences_bpps')
    os.makedirs(bpps_save_dir, exist_ok=True)

    from tqdm import tqdm  # already imported above

    for seq, seq_id in tqdm(zip(long_data.sequence.to_list(), long_data.id.to_list()), total=len(long_data),
                            desc="Regenerating BPPs (overwrite mode)"):
        bpps_path = os.path.join(bpps_save_dir, f'{seq_id}.npy')
        try:
            bpp_matrix = generate_bpp_matrix(seq)
            if bpp_matrix.ndim != 2 or bpp_matrix.shape[0] != bpp_matrix.shape[1]:
                raise ValueError(f"[{seq_id}] Invalid shape: {bpp_matrix.shape}")
            np.save(bpps_path, bpp_matrix)
        except Exception as e:
            print(f"[{seq_id}] Failed to regenerate BPP: {e}")
    ...
    if ls_indices.sum() > 0:
        ...
        original_long_dataset = RNADatasetPrecomputed(
            sequences=long_data.sequence.to_list(),
            labels=np.zeros((len(long_data), 91, opts.nclass), dtype=np.float32),
            ids=np.asarray(long_data.id.to_list()),
            error_weights=np.zeros((len(long_data), 91, opts.nclass), dtype=np.float32),  # ✅ correct
            bpps_dir=os.path.join(opts.path, 'new_sequences_bpps'),
            pad=False
        )

        long_dataset = BPPSafeDataset(original_long_dataset)
...
if ss_indices.sum() > 0:
    ...
    print("Forcing regeneration of BPP matrices for all short sequences...")

    bpps_save_dir = os.path.join(opts.path, 'new_sequences_bpps')
    os.makedirs(bpps_save_dir, exist_ok=True)

    for seq, seq_id in tqdm(zip(short_data.sequence.to_list(), short_data.id.to_list()), total=len(short_data),
                            desc="Regenerating BPPs for short sequences"):
        bpps_path = os.path.join(bpps_save_dir, f'{seq_id}.npy')
        try:
            bpp_matrix = generate_bpp_matrix(seq)
            if bpp_matrix.ndim != 2 or bpp_matrix.shape[0] != bpp_matrix.shape[1]:
    ...
    original_short_dataset = RNADatasetPrecomputed(
        sequences=short_data.sequence.to_list(),
        labels=np.zeros((len(short_data), 91, opts.nclass), dtype=np.float32),
        ids=np.asarray(short_data.id.to_list()),
        error_weights=np.zeros((len(short_data), 91, opts.nclass), dtype=np.float32),  # ✅ correct
        bpps_dir=os.path.join(opts.path, 'new_sequences_bpps'),
        pad=False
    )
...
with torch.no_grad():
    for batch in tqdm(long_dataloader, desc="Processing long sequences"):
        sequences = batch['data'].to(device).long()  # explicitly corrected to Long type
        sample_id = batch['id']
        ...
        if seq_len > 130:
            ...
            for model in fold_models:
                ...
                pred = model(batch['embedding'].to(device), bpp_single, src_mask)
