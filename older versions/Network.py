...
class RNADegformer(nn.Module):
    ...
    def __init__(self, ntoken, nclass, ninp, nhead, nhid, nlayers, kmer_aggregation, kmers, stride=1,dropout=0.5,pretrain=False,return_aw=False):
        ...
        for i in range(nlayers):
            k_size = max(1, kmers[0] - i)  # Ensure k is at least 1
            self.transformer_encoder.append(ConvTransformerEncoderLayer(ninp, nhead, nhid, dropout, k=k_size))
    ...
    def forward(self, src, mask=None,src_mask=None):
        ...
        # Handle None mask case properly
        if mask is None:
            # Create empty mask in correct format (B, 4, L, L)
            mask = torch.zeros(B, 4, L, L, device=src.device)
        elif mask.dim() == 3 and mask.size(0) == B:
            # Handle case where mask is (B, L, L) and needs to be (B, 4, L, L)
            mask = mask.unsqueeze(1).expand(B, 4, L, L)
