    ...
    for epoch in range(opts.epochs):
        ...
        for data in training_dataloader:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                outputs1, attention_maps, reps1 = model(masked_view1, bpps, src_mask)
                outputs2, _, reps2 = model(masked_view2, bpps, src_mask)
        ...
        if (epoch + 1) % opts.save_freq == 0:
            ...
            for _ in tqdm(range(5)):
                for data in val_dataloader:
                    ...
                    # Forward pass
                    with torch.no_grad():
                        outputs, attention_maps, _ = model(masked_embeddings, bpps, src_mask)
