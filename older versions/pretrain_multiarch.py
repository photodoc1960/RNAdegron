...
def train_fold():
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=opts.batch_size,
                                ...
                                collate_fn=variable_length_collate_fn)
    # checkpointing
    checkpoints_folder = f'pretrain_weights_depth{opts.nlayers}'
    ...
    for epoch in range(opts.epochs):
        ...
        for data in training_dataloader:
            ...
            mask_selection = (src[:, :, 0] != 13) & (src[:, :, 1] != 13) & (src[:, :, 2] != 13)

            # Verify src labels before calculating loss
            assert torch.all(src[:, :, 0][mask_selection] >= 0) and torch.all(src[:, :, 0][mask_selection] < 4), \
                f"Incorrect labels for output[0]: {src[:, :, 0][mask_selection].unique()}"

            assert torch.all(src[:, :, 1][mask_selection] >= 4) and torch.all(src[:, :, 1][mask_selection] < 7), \
                f"Incorrect labels for output[1]: {src[:, :, 1][mask_selection].unique()}"

            assert torch.all(src[:, :, 2][mask_selection] >= 7) and torch.all(src[:, :, 2][mask_selection] < 13), \
                f"Incorrect labels for output[2]: {src[:, :, 2][mask_selection].unique()}"
        ...
        if (epoch + 1) % opts.save_freq == 0:
            ...
            for _ in tqdm(range(5)):
                for data in val_dataloader:
                    ...
                    assert torch.all(src[:, :, 2][mask_selection] >= 7) and torch.all(
                        src[:, :, 2][mask_selection] < 13), \
                        f"Incorrect labels for output[2]: {src[:, :, 2][mask_selection].unique()}"
