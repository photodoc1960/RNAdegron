...
# Add diagnostics on test.json
print(f"Loading test file: {json_path} – exists? {os.path.exists(json_path)}")
if os.path.exists(json_path):
    print(f" size = {os.path.getsize(json_path)} bytes; first line preview = {open(json_path).readline()[:80]!r}")
...
embedding_dict = {
    str(sid): emb
    for sid, emb in zip(embedding_data["ids"], embedding_data["embeddings"])
}

graph_dist_dict = {
    str(sid): gd
    for sid, gd in zip(embedding_data["ids"],
                       embedding_data["graph_dists"])
}

nearest_p_dict = {
    str(sid): npd
    for sid, npd in zip(embedding_data["ids"],
                        embedding_data["nearest_paired"])
}

nearest_up_dict = {
    str(sid): nud
    for sid, nud in zip(embedding_data["ids"],
                        embedding_data["nearest_unpaired"])
}
...
class BPPSafeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if sample['bpp'].ndim == 4:
            sample['bpp'] = sample['bpp'][0]
        elif sample['bpp'].ndim == 2:
...
with torch.no_grad():
    ...
    for batch in tqdm(long_dataloader, desc="Processing long sequences"):
        ...
        # ─── Remove singleton frame dimension ───────────────
...
# ─── Enforce fixed sequence length of 91 positions ───
long_preds = long_preds[:, :91, :]
long_stds = long_stds[:, :91, :]

# Pad short sequences from 68 to 91 positions
if short_preds.shape[1] < 91:
    pad_length = 91 - short_preds.shape[1]
    short_preds = np.pad(short_preds, ((0, 0), (0, pad_length), (0, 0)), mode='constant', constant_values=0)
    short_stds = np.pad(short_stds, ((0, 0), (0, pad_length), (0, 0)), mode='constant', constant_values=0)
else:
    short_preds = short_preds[:, :91, :]
    short_stds = short_stds[:, :91, :]

# Zero-out uncertainty beyond position 68 for short sequences
short_stds[:, 68:, :] = 0
