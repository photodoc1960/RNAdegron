...
class RNADataset(Dataset):
    def __init__(self, seqs, labels, ids, ew, bpp_path, transform=None, training=True, pad=False, k=5, num_layers=6, deltaG_lookup=None):
        ...
        id_to_idx = {sid: idx for idx, sid in enumerate(ids_all)}

        self.graph_dists_list = [graph_all[id_to_idx[sid]] for sid in self.ids]
        self.nearest_p_list = [npd_all[id_to_idx[sid]] for sid in self.ids]
        self.nearest_up_list = [nud_all[id_to_idx[sid]] for sid in self.ids]
    ...
    def __getitem__(self, idx):
        ...
        if self.training:
            ...
            if random.random() < 0.5:
                ...
                # flip structural features
                sample['graph_dist'] = np.flip(sample['graph_dist'], axis=0).copy()
